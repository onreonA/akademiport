...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' AND OLD.company_id != NEW.company_id THEN
    UPDATE companies 
    SET current_users = current_users - 1 
    WHERE id = OLD.company_id;
...
CREATE TRIGGER update_company_user_count_trigger
  ...
  FOR EACH ROW 
  WHEN (OLD.company_id IS NOT NULL OR NEW.company_id IS NOT NULL)
...
BEGIN
  IF NEW.slug IS NULL OR NEW.slug = '' THEN
    ...
    NEW.slug = regexp_replace(
      regexp_replace(
        regexp_replace(NEW.slug, '[öÖ]', 'o', 'g'),
        '[şŞ]', 's', 'g'
      ),
      '[üÜ]', 'u', 'g'
    );
    ...
    -- Unique slug için suffix ekle
    IF EXISTS (SELECT 1 FROM programs WHERE slug = NEW.slug AND id != COALESCE(NEW.id, '00000000-0000-0000-0000-000000000000')) THEN
