...
export const UpdateCompanySchema = z.object({
  name: z.string().min(2, 'Firma adı en az 2 karakter olmalıdır').max(100, 'Firma adı en fazla 100 karakter olabilir').optional(),
  ...
  foundationYear: z.number().int('Kuruluş yılı tam sayı olmalıdır').min(1800, 'Kuruluş yılı 1800 veya daha büyük olmalıdır').max(new Date().getFullYear()).optional(),
  logoUrl: z.string().url('Geçerli bir logo URL\'si giriniz').optional(),
  isActive: z.boolean().optional(),
  maxUsers: z.number().int('Maksimum kullanıcı sayısı tam sayı olmalıdır').min(1, 'En az 1 kullanıcı olmalıdır').max(10, 'En fazla 10 kullanıcı olabilir').optional(),
