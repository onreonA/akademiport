/**
 * Unit Tests for ApproveTopicUseCase
 */
...
describe('ApproveTopicUseCase', () => {
  ...
  let useCase: ApproveTopicUseCase;
  ...
  beforeEach(() => {
    ...
    useCase = new ApproveTopicUseCase(mockRepository);
  });
  ...
  describe('execute', () => {
    it('should approve topic successfully', async () => {
      ...
      expect(mockRepository.approveTopic).toHaveBeenCalledWith(topicId);
    });

    it('should handle repository failure', async () => {
      const topicId = 'topic-1';

      vi.mocked(mockRepository.approveTopic).mockResolvedValue(Result.fail('Database error'));
    ...
    it('should handle errors gracefully', async () => {
      ...
      vi.mocked(mockRepository.approveTopic).mockRejectedValue(new Error('Unexpected error'));
