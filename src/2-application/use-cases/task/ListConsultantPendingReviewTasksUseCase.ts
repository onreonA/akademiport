...
/**
 * List Consultant Pending Review Tasks Use Case
 *
 * Consultant'ın atandığı projelerdeki 'review' durumundaki görevleri listeler
 */
export class ListConsultantPendingReviewTasksUseCase {
  constructor(
  ...
  async execute(consultantId: string): Promise<Result<Task[]>> {
    try {
      ...
      // 3. Bu sub-projelerdeki 'review' durumundaki görevleri bul (sub-project, project, company bilgileriyle birlikte)
      ...
      if (error) {
        throw new Error(`Failed to find pending review tasks: ${error.message}`);
      }
      ...
      return Result.fail(
        new AppError(
          error instanceof Error ? error.message : 'Failed to list pending review tasks',
          500
        )
      );
    }
