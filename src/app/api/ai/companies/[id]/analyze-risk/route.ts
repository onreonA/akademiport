/**
 * POST /api/ai/companies/[id]/analyze-risk
 *
...
import { AnalyzeCompanyRiskUseCase } from '@/2-application/use-cases/ai/AnalyzeCompanyRiskUseCase';
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Create use case
    const useCase = new AnalyzeCompanyRiskUseCase(
      aiRouter,
      promptManager,
      tokenTracker,
      projectRepository,
    ...
    if (result.isFailure) {
      logger.error('Failed to analyze company risk:', result.error);
      return NextResponse.json(
        {
          error:
            result.error instanceof AppError
              ? result.error.message
              : 'Failed to analyze company risk',
        },
        {
    ...
    logger.error('Error in POST /api/ai/companies/[id]/analyze-risk:', error);
